...
class BlogPostController extends AController
{
                ...
                if ($url->save(false)) {
                    ...
                    if ($model->save(false)) {
                        ...
                        Yii::app()->user->setFlash(
                            'success',
                            Yii::t('admin', 'The post {title} has been created!', array('{title}' => $model->title))
                        );
	...
	public function actionUpdate($id)
	{
		...
		if (isset($_POST['Page'])) {
            ...
            if ($model->validate() && $url->validate()) {
                ...
                Yii::app()->user->setFlash(
                    'success',
                    Yii::t('admin', 'The post {title} has been updated!', array('{title}' => $model->title))
                );
	 ...
	 * If deletion is successful, the browser will be redirected to the 'admin' post.
	...
	public function actionDelete($id)
	{
        ...
        Yii::app()->user->setFlash(
            'success',
            Yii::t('admin', 'The post {title} has been deleted!', array('{title}' => $model->title))
        );
    ...
    {
        $categories = array();
        foreach (Category::model()->blog()->findAll() as $category) {
